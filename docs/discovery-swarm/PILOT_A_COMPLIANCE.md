...
| Item | Status | Evidence / test | Remediation |
|---|---|---|---|
| Production endpoint completes initialize handshake | **PASS** (was **FAIL**) | At audit start: `421 Misdirected Request` on every request (unpinned fastmcp Host guard). Fixed e8749bd (emergency global disable), then narrowed to an explicit Host/Origin allowlist (`147d602`: `host_origin_protection="auto"` + `GUILD_PUBLIC_HOSTS`) — spoofed Host again 421, foreign browser Origin 403, public host verified 200 in production. Full evidence chain (exact failing request, complete 421 response, configured vs accepted values in the installed fastmcp 3.4.4, file/line, security-consequence table): `evidence/mcp-421-host-guard.md`. Complete production handshake battery (initialize → initialized → tools/list → tools/call → malformed → unauthorised): `harness/results/mcp_battery_prod.txt`; regression tests `tests/test_mcp_host_guard.py`. | Keep fastmcp pinned; add an external MCP-handshake canary to ops-watch. |
...
| Internal / test / external callers distinguishable | **PARTIAL** — 7-class taxonomy live (`caller_classes` in `/instrumentation`). The post-deploy verification found the two classifiers disagreeing (the MCP battery was AG_TEST under `caller_class` yet still counted in the `genuine_external` headline); fixed same day: `is_genuine_external` now consults the AG-test and crawler UA rules, and `attribution_class` labels them `ag_test`/`registry_crawler` (classification is read-time, so history heals retroactively). Residual weaknesses: honor-based first-party header (no `GUILD_FIRST_PARTY_TOKEN` set), generic framework UAs (httpx) count as genuine, incidents are manual. |
