...
FETCH_READ_TIMEOUT_S = 10.0            # bounded read (connect stays 3s)
...
def adapter_x402_bazaar(capability: str, fetch: Callable) -> list[dict]:
    ...
    resources. The Bazaar ITEM is the candidate's manifest; its `resource`
    is the endpoint."""
...
def qualify_candidate(cand: dict, fetch: Callable,
    ...
    """Validate the candidate's discovery evidence, verify endpoint
    reachability with a probe that speaks the candidate's DECLARED protocol,
    and record declared bindings (never trusted). Returns the candidate
    record — ALWAYS classified `discovered_unverified`: being discoverable
    is not evidence.

      * a2a  — fetch + validate the public agent card; A2A liveness probe;
...
def run_scout(store: Any, fetch: Callable = safe_fetch_json,
    ...
    summary: dict[str, Any] = {
        ...
        "adapters": {name: {"candidates": 0, "ok": True}
    ...
    for row in demand_rows:
        ...
        for name, adapter in adapters.items():
            ...
            try:
                ...
                summary["adapters"][name]["ok"] = False
                summary["adapters"][name]["reason"] = f"{type(e).__name__}"
                continue
            summary["adapters"][name]["candidates"] += len(cands)
            for cand in cands:
                cand["capability"] = cap
