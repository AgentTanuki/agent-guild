...
``tools/list`` verbatim, and INTERCEPTS every ``tools/call``:
  ...
  1. gate(capability=<tool name or downstream-declared capability>)
  2. denied  -> the host receives a structured policy denial (isError=true),
     and a signed 'blocked' outcome is recorded
  3. allowed -> forwarded; the result is hashed and a signed outcome recorded

This is discovery-and-invocation interception for the MCP ecosystem — not a
lookup tool the model may or may not call.
"""
...
def build_proxy(gateway: Gateway, downstream_cmd: list[str],
                ...
                capability_map: dict[str, str] | None = None) -> Server:
