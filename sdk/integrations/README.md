...
|-----------|------|-------|
| **A2A machine-message receiver** | [`machine_envelope_receiver.mjs`](machine_envelope_receiver.mjs) | Build the fail-closed verifier with `createAgentGuildMachineEnvelopeReceiver(...)`, then package it with `createAgentGuildA2AEnvelopeExtension({receiver, acquisitionUrl, kind})`. The adapter publishes an Agent Card entry, activates only through the official `A2A-Extensions` header, echoes accepted activation, verifies exact payload/issuer/recipient/purpose/expiry/message-id binding, and atomically consumes one paid envelope before consequential side effects. `Message.extensions` is optional descriptive data and remains signed when present. The bundled replay store is process-local; distributed agents provide a durable atomic store. |
...
| **Taskmarket requester** | [`taskmarket_requester.mjs`](taskmarket_requester.mjs) | `createAgentGuildTaskmarketRequester({evmSigner, mandateId, approve})`; preview exact task/reward/deadline/deliverables, require fresh approval, buy a signed delegation envelope, enforce the AGSM-1 cap before Taskmarket funding, then expose read-only status/submission review with no automatic acceptance and no blind payment retry. |
